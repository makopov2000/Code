...
public class Streams1 {
 ...
 // Unique entries
        List<String> x = stringCollection.stream()
                .distinct()
                .sorted()
 ...
 Set<String> dupResult = stringCollection.stream()
                    .filter(l -> !process.add(l))
                    .collect(Collectors.toSet());
        System.out.println("Duplicate list: "+dupResult);

        Set<String> secondEx = stringCollection.stream()
                .filter(s -> stringCollection.stream().filter(z -> z == s).count() > 1)
                .collect(Collectors.toSet());
        ...
        Set<Integer> duplicated = numbers.stream()
                .filter(n -> numbers.stream().filter(z -> z == n).count() > 1)
                .collect(Collectors.toSet());
